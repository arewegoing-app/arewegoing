...
import { eventFeedback, eventInvites, events } from '@/lib/db/schema';
import { and, eq, isNull } from 'drizzle-orm';
import { applyTokenRsvp } from '@/lib/rsvp/actions';
import { applyPledgeToken } from '@/lib/rsvp/pledge';
import { applyBailRequestToken, applyResaleClaimToken } from '@/lib/rsvp/resale';
import { applyReactionToken } from '@/lib/discovery/reactions';
import { verifyToken } from '@/lib/tokens/token-service';
...
export async function GET(req: NextRequest) {
  ...
  if (v.payload.act === 'feedback.submit') {
    ...
    if (rating === null || isNaN(rating) || rating < 0 || rating > 5) {
      ...
      return NextResponse.redirect(new URL('/r/error?reason=malformed', req.url));
    }
    ...
    return NextResponse.redirect(new URL('/feedback/thanks', req.url));
  }
